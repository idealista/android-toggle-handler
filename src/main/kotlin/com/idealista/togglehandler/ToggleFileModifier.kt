...
class ToggleFileModifier(private val project: Project) {
    ...
    fun addToggleDefinition(file: PsiFile, toggleData: ToggleDefinitions.ToggleData) {
        ...
        val newToggleDefinition = """    data object ${toggleData.name} : Toggle(
        name = "$snakeCaseName",
...
"""
    ...
    fun addRemoteSettingsDefaults(file: PsiFile, toggleData: ToggleDefinitions.ToggleData) {
        ...
        while (currentIndex < fileText.length && openParents > 0) {
            when (fileText[currentIndex]) {
                '(' -> openParents++
                ')' -> openParents--
                ',' -> if (openParents == 1) hasElements = true
            }

            if (openParents == 1 && fileText[currentIndex].isLetterOrDigit()) {
                hasElements = true
            }
            currentIndex++
        }
        ...
        val closingParentIndex = currentIndex - 1

        WriteCommandAction.runWriteCommandAction(project) {
            try {
                if (hasElements) {
                    var insertIndex = closingParentIndex
                    ...
                    document.insertString(closingParentIndex, "\n        Toggle.${toggleData.name}.name to false,\n    ")
                }
                PsiDocumentManager.getInstance(project).commitDocument(document)
            } catch (e: Exception) {
                Messages.showErrorDialog(
